...
node[:deploy].each do |application, deploy|
  ...
  Chef::Log.info("docker run --net=host -e AWS_SECRET_ACCESS_KEY=#{node[:custom_env][:common][:AWS_SECRET_ACCESS_KEY]} -e AWS_ACCESS_KEY_ID=#{node[:custom_env][:common][:AWS_ACCESS_KEY_ID]} --name spark -e SPARK_MASTER_IP=#{node[:opsworks][:instance][:private_ip]} -e SPARK_LOCAL_HOSTNAME=#{node[:opsworks][:instance][:hostname]} -e HOSTNAME=#{node[:opsworks][:instance][:private_ip]} -d vingle/spark 'cd /usr/local/spark/ ; sbin/start-master.sh'")
  bash "docker-run" do
    user "root"
    cwd "#{deploy[:deploy_to]}"
    code <<-EOH
