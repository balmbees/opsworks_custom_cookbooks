...
node[:deploy].each do |application, deploy|
  ...
  Chef::Log.info("docker run #{dockerenvs} --name unicorn_rails -h #{node[:opsworks][:instance][:hostname]} -v /mnt/var/log/nginx:/var/log/nginx --network=host --pid=host --privileged -d #{node[:docker][:DOCKER_RAILS_REPO]} --log-opt max-size=100m")
  bash "docker-run" do
    user "root"
    ...
    code <<-EOH
      ...
      then
        ...
        docker run #{dockerenvs} --name unicorn_rails -h #{node[:opsworks][:instance][:hostname]} -v /mnt/var/log/nginx:/var/log/nginx --network=host --pid=host --privileged -d #{node[:docker][:DOCKER_RAILS_REPO]} --log-opt max-size=100m
        sleep 3
      fi
    EOH
  end
