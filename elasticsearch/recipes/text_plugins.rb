script "install_plugin_es" do
  ...
  if node[:elasticsearch][:version].to_f >= 2.0
    ...
    code_plugins = <<-EOH
      ...
      bin/plugin -i jdbc -u http://xbib.org/repository/org/xbib/elasticsearch/plugin/elasticsearch-river-jdbc/#{node[:elasticsearch][:plugin]['elasticsearch-jdbc']}/elasticsearch-river-jdbc-#{node[:elasticsearch][:plugin]['elasticsearch-jdbc']}-plugin.zip
      wget https://jdbc.postgresql.org/download/#{node[:elasticsearch][:plugin]['pg-jdbc-driver']}
      mv #{node[:elasticsearch][:plugin]['pg-jdbc-driver']} plugins/jdbc/
